...
[Feature("Behavioral - Visitor Concurrency (Result)")]
public sealed class VisitorResultConcurrencyTests(ITestOutputHelper output) : TinyBddXunitBase(output)
{
    private sealed record State(Visitor<Node, int> v, int[] counters, Node[] nodes, int perType);
        ...
        => Given("a result visitor and a mixed dataset", () =>
            {
                ...
                var v = Visitor<Node, int>
                    .Create()
                    .On<Add>(static _ => 0)
                    .On<Number>(static _ => 1)
                    .Default(static _ => 2)
                    .Build();
           ...
           .When("visiting in parallel and tallying results", VisitInParallel)
