...
public sealed class PricingDemoTests(ITestOutputHelper output) : TinyBddXunitBase(output)
{
    ...
    public async Task EndToEnd_Sample()
    {
        await Given("default artifacts and context with loyalty + coupon", () =>
        {
            ...
            var ctx = BaseContext(PaymentKind.Cash);
        ...
        .And("applied bundle and coupon entries if eligible", r => r.Log.Any(l => l.StartsWith("bundle:BNDL")) && r.Log.Any(l => l.StartsWith("coupon:CASHOFF1")))
    ...
    public async Task Loyalty_Stacking_Exclusive()
    {
        ...
        .Then("loyalty:LOY-10X appears and blocks another on same sku at least once", tr => tr.Item2.Log.Any(l => l.Contains("loyalty:LOY-10X:SKU-APPLE")))
        .AssertPassed();
    }
    ...
    public async Task Rounding_Priority_Charity_Over_Nickel()
    {
        ...
        .When("run", RunPipeline)
        .Then("round:charity-up present, no round:nickel", r => r.Log.Any(l => l.StartsWith("round:charity-up")) && !r.Log.Any(l => l.StartsWith("round:nickel")))
        .AssertPassed();
    }
