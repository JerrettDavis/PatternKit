...
[Generator]
public sealed class IdentityMapGenerator : IIncrementalGenerator
{
    private const string GenerateAttributeName = "PatternKit.Generators.IdentityMap.GenerateIdentityMapAttribute";
    private const string KeySelectorAttributeName = "PatternKit.Generators.IdentityMap.IdentityMapKeySelectorAttribute";

    private static readonly DiagnosticDescriptor MustBePartial = new(
        "PKIM001", "Identity Map host must be partial",
        "Type '{0}' is marked with [GenerateIdentityMap] but is not declared as partial",
        "PatternKit.Generators.IdentityMap", DiagnosticSeverity.Error, true);

    private static readonly DiagnosticDescriptor MissingKeySelector = new(
        "PKIM002", "Identity Map key selector is missing",
        "Identity Map '{0}' must declare exactly one [IdentityMapKeySelector] method",
        "PatternKit.Generators.IdentityMap", DiagnosticSeverity.Error, true);

    private static readonly DiagnosticDescriptor InvalidKeySelector = new(
        "PKIM003", "Identity Map key selector signature is invalid",
        "Identity Map key selector '{0}' must be static and return TKey from one TEntity parameter",
        "PatternKit.Generators.IdentityMap", DiagnosticSeverity.Error, true);
    ...
    private static void Generate(SourceProductionContext context, INamedTypeSymbol type, TypeDeclarationSyntax node, AttributeData attribute)
    {
        ...
        context.AddSource($"{type.Name}.IdentityMap.g.cs", SourceText.From(
