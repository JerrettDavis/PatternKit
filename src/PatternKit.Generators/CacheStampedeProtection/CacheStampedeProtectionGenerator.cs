...
[Generator]
public sealed class CacheStampedeProtectionGenerator : IIncrementalGenerator
{
    private const string AttributeName = "PatternKit.Generators.CacheStampedeProtection.GenerateCacheStampedeProtectionAttribute";
    ...
    private static readonly DiagnosticDescriptor MustBePartial = new(
        "PKCSP001",
        "Cache Stampede Protection host must be partial",
        "Type '{0}' is marked with [GenerateCacheStampedeProtection] but is not declared as partial",
        "PatternKit.Generators.CacheStampedeProtection",
        DiagnosticSeverity.Error,
        true);

    private static readonly DiagnosticDescriptor InvalidConfiguration = new(
        "PKCSP002",
        "Cache Stampede Protection configuration is invalid",
        "Cache Stampede Protection '{0}' must have non-empty FactoryMethodName and PolicyName values",
        "PatternKit.Generators.CacheStampedeProtection",
        DiagnosticSeverity.Error,
        true);
    ...
    private static void Generate(SourceProductionContext context, INamedTypeSymbol type, TypeDeclarationSyntax node, AttributeData attribute)
    {
        ...
        var policyName = GetNamedString(attribute, "PolicyName") ?? "cache-stampede-protection";
        if (string.IsNullOrWhiteSpace(factoryMethodName) || string.IsNullOrWhiteSpace(policyName))
        {
        ...
        context.AddSource($"{type.Name}.CacheStampedeProtection.g.cs", SourceText.From(
            GenerateSource(type, resultType, factoryMethodName, policyName),
            Encoding.UTF8));
    }

    private static string GenerateSource(INamedTypeSymbol type, INamedTypeSymbol resultType, string factoryMethodName, string policyName)
    {
        ...
        sb.Append(memberIndent).Append("public static global::PatternKit.Cloud.CacheStampedeProtection.CacheStampedeProtectionPolicy<").Append(resultTypeName).Append("> ").Append(factoryMethodName).AppendLine("()");
        sb.Append(memberIndent).AppendLine("{");
        sb.Append(bodyIndent).Append("return global::PatternKit.Cloud.CacheStampedeProtection.CacheStampedeProtectionPolicy<").Append(resultTypeName).Append(">.Create(\"").Append(Escape(policyName)).AppendLine("\").Build();");
        sb.Append(memberIndent).AppendLine("}");
        sb.Append(indent).AppendLine("}");
