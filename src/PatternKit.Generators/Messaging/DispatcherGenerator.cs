...
public sealed class DispatcherGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateObjectStreamMethod(StringBuilder sb)
    {
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        // Use dynamic to enumerate the IAsyncEnumerable");
        sb.AppendLine("        dynamic asyncEnumerable = result;");
        sb.AppendLine("        var enumerator = asyncEnumerable.GetAsyncEnumerator(ct);");
        sb.AppendLine();
        sb.AppendLine("        try");
        sb.AppendLine("        {");
        sb.AppendLine("            while (await enumerator.MoveNextAsync())");
        sb.AppendLine("            {");
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            await enumerator.DisposeAsync();");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine();
    }
