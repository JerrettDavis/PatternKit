...
public sealed class DispatcherGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateObjectStreamMethod(StringBuilder sb)
    {
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            var enumeratorType = result.GetType();");
        sb.AppendLine("            var getAsyncEnumeratorMethod = enumeratorType.GetMethod(\"GetAsyncEnumerator\", new[] { typeof(CancellationToken) });");
        sb.AppendLine("            if (getAsyncEnumeratorMethod != null)");
        sb.AppendLine("            {");
        sb.AppendLine("                var enumerator = getAsyncEnumeratorMethod.Invoke(result, new object[] { ct });");
        sb.AppendLine("                if (enumerator != null)");
        sb.AppendLine("                {");
        sb.AppendLine("                    var moveNextAsyncMethod = enumerator.GetType().GetMethod(\"MoveNextAsync\");");
        sb.AppendLine("                    var currentProperty = enumerator.GetType().GetProperty(\"Current\");");
        ...
        sb.AppendLine("                            {");
        sb.AppendLine("                                if (!await vtBool) break;");
