...
public sealed class VisitorGenerator : IIncrementalGenerator
{
    ...
    private static void GenerateAcceptMethods(SourceProductionContext context, VisitorRootInfo root)
    {
        ...
        {
            var hidesBaseAcceptMethods = type.BaseType is not null
                && allTypes.Any(baseType => SymbolEqualityComparer.Default.Equals(baseType, type.BaseType));
            var memberHidingModifier = hidesBaseAcceptMethods ? "new " : "";
            ...
            sb.AppendLine($"    public {memberHidingModifier}TResult Accept<TResult>({root.VisitorInterfaceName}<TResult> visitor)");
            sb.AppendLine("    {");
            ...
            if (root.GenerateActions)
            {
                ...
                sb.AppendLine($"    public {memberHidingModifier}void Accept({root.VisitorInterfaceName}Action visitor)");
                sb.AppendLine("    {");
            ...
            if (root.GenerateAsync)
            {
                ...
                sb.AppendLine($"    public {memberHidingModifier}System.Threading.Tasks.ValueTask<TResult> AcceptAsync<TResult>({root.VisitorInterfaceName}Async<TResult> visitor, System.Threading.CancellationToken cancellationToken = default)");
                sb.AppendLine("    {");
                ...
                if (root.GenerateActions)
                {
                    ...
                    sb.AppendLine($"    public {memberHidingModifier}System.Threading.Tasks.ValueTask AcceptAsync({root.VisitorInterfaceName}AsyncAction visitor, System.Threading.CancellationToken cancellationToken = default)");
                    sb.AppendLine("    {");
                    sb.AppendLine($"        return visitor.VisitAsync(this, cancellationToken);");
                    sb.AppendLine("    }");
                    sb.AppendLine();
