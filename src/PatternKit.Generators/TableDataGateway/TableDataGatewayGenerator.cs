...
[Generator]
public sealed class TableDataGatewayGenerator : IIncrementalGenerator
{
    private const string GenerateAttributeName = "PatternKit.Generators.TableDataGateway.GenerateTableDataGatewayAttribute";
    private const string KeySelectorAttributeName = "PatternKit.Generators.TableDataGateway.TableGatewayKeySelectorAttribute";

    private static readonly DiagnosticDescriptor MustBePartial = new(
        "PKTDG001", "Table Data Gateway host must be partial",
        "Type '{0}' is marked with [GenerateTableDataGateway] but is not declared as partial",
        "PatternKit.Generators.TableDataGateway", DiagnosticSeverity.Error, true);

    private static readonly DiagnosticDescriptor MissingKeySelector = new(
        "PKTDG002", "Table Data Gateway key selector is missing",
        "Table Data Gateway '{0}' must declare exactly one [TableGatewayKeySelector] method",
        "PatternKit.Generators.TableDataGateway", DiagnosticSeverity.Error, true);

    private static readonly DiagnosticDescriptor InvalidKeySelector = new(
        "PKTDG003", "Table Data Gateway key selector signature is invalid",
        "Table Data Gateway key selector '{0}' must be static and return TKey from one TRow parameter",
        "PatternKit.Generators.TableDataGateway", DiagnosticSeverity.Error, true);
    ...
    private static void Generate(SourceProductionContext context, INamedTypeSymbol type, TypeDeclarationSyntax node, AttributeData attribute)
    {
        ...
        var rowType = attribute.ConstructorArguments.Length > 0 ? attribute.ConstructorArguments[0].Value as INamedTypeSymbol : null;
        var keyType = attribute.ConstructorArguments.Length > 1 ? attribute.ConstructorArguments[1].Value as INamedTypeSymbol : null;
        if (rowType is null || keyType is null)
            return;
        ...
        if (!IsKeySelector(selector, rowType, keyType))
        {
            context.ReportDiagnostic(Diagnostic.Create(InvalidKeySelector, selector.Locations.FirstOrDefault(), selector.Name));
            return;
        }

        var tableName = GetNamedString(attribute, "TableName");
        if (string.IsNullOrWhiteSpace(tableName))
            tableName = type.Name;

        context.AddSource($"{type.Name}.TableDataGateway.g.cs", SourceText.From(
            GenerateSource(type, rowType, keyType, selector.Name, GetNamedString(attribute, "FactoryName") ?? "Create", tableName!),
            Encoding.UTF8));
    }
        ...
        string tableName)
    {
        ...
        var rowName = rowType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
    ...
    private static bool IsKeySelector(IMethodSymbol method, INamedTypeSymbol rowType, INamedTypeSymbol keyType)
        ...
        && SymbolEqualityComparer.Default.Equals(method.Parameters[0].Type, rowType)
